        ...
        <div class="flex items-center pt-8 pl-5 space-x-2 border-t border-gray-100">
            {{-- Validation Photo --}}
                @if (auth()->user()->detail_user()->first()->photo != NULL)
                    <img class="inline ml-3 h-12 w-12 rounded-full" src="{{ url(Storage::url(auth()->user()->detail_user()->first()->photo)) }}" alt="Photo" loading="lazy" />
                @else
        ...
        <ul>
            ...
            <li class="relative px-6 py-3">
                ...
                <a class="inline-flex items-center w-full text-sm font-light transition-colors duration-150 hover:text-gray-800"
                    ...
                    <span
                        class="inline-flex items-center justify-center px-3 py-2 ml-auto text-xs font-bold leading-none text-green-500 rounded-full bg-serv-green-badge">{{ auth()->user()->order_freelance()->count() }}</span>
