    ...
    <div class="w-96 h-auto overflow-hidden md:p-5 p-4 bg-white rounded-2xl inline-block">
        <div class="flex items-center space-x-2 mb-6">
            ...
            @if ($item->user->detail_user->photo != NULL)
                <img class="object-cover w-14 h-14 rounded-full mr-1" src="{{ url(Storage::url($item->user->detail_user->photo)) }}" alt="Photo Freelancer" loading="lazy">
            @else
            <svg class="object-cover object-center w-14 h-14 rounded-full mr-1 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
